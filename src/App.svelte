<script lang='typescript'>
	...
	const checkIdVid = async() => {
		...
		try{
			...
			const resId= await fetch(`${(import.meta.build.env.SNOWPACK_PUBLIC_USER_URL)}userId/${uId}/${vidDate}.json`)
			idData = await resId.json()
			// Checking if the VideoID is true
			const resVid = await fetch(`${(import.meta.build.env.SNOWPACK_PUBLIC_VIDEO_URL)}videoLink/${vidDate}/${$params.v}.json`)
			...
			if( !isEmpty(vidData) && !isEmpty(idData) && idData.groups.includes($params.v)){
				...
				const secondResId = await fetch(`${(import.meta.build.env.SNOWPACK_PUBLIC_USER_URL)}userId/${uId}/${vidDate}.json`,{
					method: 'PATCH',
				...
				const secondResVid = await fetch(`${(import.meta.build.env.SNOWPACK_PUBLIC_VIDEO_URL)}videoLink/${vidDate}/${$params.v}.json`,{
					method: 'PATCH',
				...
				sUrl = `${(import.meta.build.env.SNOWPACK_PUBLIC_DATABASE_URL)}data/${dataDate}/${videoId}/${uId}.json`
				linkValid = true
			}else{
				linkValid = false
	...
	const sendFirstData = () => {
		...
		.then(data => {
			...
			sUrl = `${(import.meta.build.env.SNOWPACK_PUBLIC_DATABASE_URL)}data/${dataDate}/${videoId}/${uId}/${$allData.id}.json`
