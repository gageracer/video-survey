<script lang='typescript'>
	...
	const checkIdVid = async() => {
		...
		try{
			...
			if( !isEmpty(vidData) && !isEmpty(idData) && idData.groups.includes($params.v)){
				...
				const secondResId = await fetch(`${speDec(import.meta.env.SNOWPACK_PUBLIC_USER_URL)}userId/${uId}/${vidDate}.json`,{
					method: 'PATCH',
				...
				const secondResVid = await fetch(`${speDec(import.meta.env.SNOWPACK_PUBLIC_VIDEO_URL)}videoLink/${vidDate}/${$params.v}.json`,{
					method: 'PATCH',
				...
				sUrl = `${speDec(import.meta.env.SNOWPACK_PUBLIC_DATABASE_URL)}data/${dataDate}/${videoId}/${uId}.json`
				linkValid = true
			}else{
				linkValid = false
	...
	const sendFirstData = () => {
		...
		.then(data => {
			...
			sUrl = `${speDec(import.meta.env.SNOWPACK_PUBLIC_DATABASE_URL)}data/${dataDate}/${videoId}/${uId}/${$allData.id}.json`
